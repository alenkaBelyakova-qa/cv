...
while(!isSunk) {
	let guess = prompt("Привет! Это мини-игра морской бой :D Представь, что в 7 ячейках по горизонтали ••••••• (от нуля до шести) расположен 3-ёх палубный корабль врага. Твоя задача потопить его за меньшее кол-во попыток. Стреляй! (введи число 0-6):", "");
	if (guess == null) {
		alert("Жаль,что вы покинули игру :(");
		break;
	} else if (isNaN(guess) || guess < 0 || guess > 6) {
		alert("Ошибка ввода! Вы должны ввести число от 0 до 6");
